...
async def extract_terabox_info(url: str):
    try:
        async with httpx.AsyncClient(timeout=60.0) as client:
            ...
            data = response.json()
            logger.info(f"API response keys for {url}: {list(data.keys())}")
...
Use /status to check your current status.
        ...
        try:
            ...
            for idx, file_info in enumerate(files, 1):
                ...
                logger.info(f"File {idx}: name={file_name}, type={file_type}, keys={list(file_info.keys())}")
            ...
            if videos_downloaded > 0:
                ...
                await status_message.edit_text("❌ No files could be downloaded. Please try again or check the link.")
    ...
    while True:
        try:
            ...
            try:
                ...
                try:
                    client = AsyncIOMotorClient(MONGO_URL)
                    db_new = client['terabox_bot']
                    users_collection.__init__(db_new, 'users')
