...
async def send_video_to_user(update: Update, context: ContextTypes.DEFAULT_TYPE,
    ...
    fast_stream_url = raw_fast if isinstance(raw_fast, str) else None

    download_url = (
        file_info.get("normal_dlink") or
        file_info.get("stream_url") or
        fast_stream_url or          # only if it's actually a plain string URL
    ...
    # ── Download file to disk (streaming chunks) ──────────────────────────────
    ...
    if not local_path:
        await update.message.reply_text(
            ...
            f"The link may have expired — please try again."
        )
        return False
